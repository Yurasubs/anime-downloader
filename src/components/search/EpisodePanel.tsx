...
const LANG_LABELS: Record<string, string> = {
  ...
  'und': 'UND', 'jpn': 'JA', 'eng': 'EN', 'spa': 'ES', 'spa-419': 'ES-LA',
  'spa-es': 'ES-ES', 'por': 'PT', 'fra': 'FR', 'deu': 'DE', 'ara-me': 'AR-ME',
  'ara': 'AR', 'ita': 'IT', 'rus': 'RU', 'tur': 'TR', 'hin': 'HI',
  'zho': 'ZH', 'chi': 'ZH', 'zh-hk': 'ZH-HK', 'kor': 'KO', 'cat': 'CA',
  'pol': 'PL', 'tha': 'TH', 'tam': 'TA', 'may': 'MS', 'vie': 'VI',
  'ind': 'ID', 'tel': 'TE',
  // Subtitle locale codes (BCP-47 style)
  'all': 'All', 'none': 'None',
  'en-us': 'EN', 'en-in': 'EN-IN', 'es-419': 'ES-LA', 'es-es': 'ES-ES',
  'pt-br': 'PT-BR', 'pt-pt': 'PT-PT', 'fr-fr': 'FR', 'de-de': 'DE',
  'ar-me': 'AR', 'ar-sa': 'AR-SA', 'it-it': 'IT', 'ru-ru': 'RU', 'tr-tr': 'TR',
  'hi-in': 'HI', 'zh-cn': 'ZH-CN', 'zh-tw': 'ZH-TW',
  'ko-kr': 'KO', 'ca-es': 'CA', 'pl-pl': 'PL', 'th-th': 'TH', 'ta-in': 'TA',
  'ms-my': 'MS', 'vi-vn': 'VI', 'id-id': 'ID', 'te-in': 'TE', 'ja-jp': 'JA',
};
...
export default function EpisodePanel() {
  ...
  const handleAddToQueue = async () => {
    ...
    const episodeSelector = selected.size === episodes.length
      ? ''
    ...
    try {
      ...
      addNotification(
        `Added ${selected.size} episode(s) to queue`,
        'success'
      );
    } catch {
        ...
        <div className="flex items-start gap-3">
          {selectedSeries?.image && (
            <img
              ...
              className="w-12 aspect-[2/3] rounded-lg object-cover flex-shrink-0"
            />
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setDownloadOption('q', parseInt(e.target.value))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setDownloadOption('fileName', e.target.value)
                }
            ...
            {[
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setDownloadOption(key, e.target.checked)
                  }
    ...
    >
      ...
      {episode.img && (
        <img
          src={episode.img}
          alt=""
          className="w-20 h-12 rounded object-cover flex-shrink-0 bg-background"
          loading="lazy"
        />
      )}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          {episode.lang?.map((l) => (
            <span
              key={l}
              className="text-[9px] px-1 py-0.5 rounded bg-background text-muted uppercase"
            >
              {langLabel(l)}
            </span>
          ))}
          {episode.time && (
            <span className="text-[10px] text-muted/50">{episode.time}</span>
          )}
        </div>
      </div>
    </button>
