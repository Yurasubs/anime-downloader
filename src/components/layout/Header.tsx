...
import { Search, ListOrdered, LogOut, Loader2, User, Settings } from "lucide-react"
import type { Tab } from "./MainLayout"
import AuthDialog from "@/components/ui/AuthDialog"

export default function Header({ activeTab, onTabChange }: { activeTab: Tab; onTabChange: (tab: Tab) => void }) {
                ...
                <div className="max-w-screen-2xl mx-auto px-4 md:px-6">
                    <div className="grid grid-cols-3 items-center h-16">
                        ...
                        <div className="flex items-center gap-3 justify-start">
                            ...
                            {service && (
                                <span className={`text-[15px] font-medium ${serviceColors[service] || "text-primary"}`}>
                                    {serviceNames[service] || service}
                                </span>
                            )}
                            {version && (
                                <span className="text-[13px] text-muted-foreground hidden sm:inline">v{version}</span>
                            )}
                        </div>
